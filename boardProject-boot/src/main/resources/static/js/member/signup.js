...
//필수 입력 항목의 유효성 검사 여부를 체크하기 위한 SJ객체
const checkObj = {
    "memberEmail"       : false,
    "memberPw"          : false,
...
// 2) 이메일이 입력될(input) 때 마다 유효성 검사 수행
memberEmail.addEventListener("input", (e)=> {
    ...
    // 영어, 숫자, 특수문자(. _ % + -)를 허용
    // 이메일 형식으로 작성
    // 도메인 부분은 최소 2개의 문자로 끝나야 함(.com, .net, .org, .kr등)

    // 입력받은 이메일이 정규식과 일치하지 않는 경우
    // == 알맞은 이메일 형태가 아닌경우 
    if(!regExp.test(inputEmail)){
        emailMessage.innerText="알맞은 이메일 양식으로 작성해주세요."
        emailMessage.classList.add('error') // 글자를 빨간색으로 변경
        emailMessage.classList.remove('confirm') // 초록색 글씨 제가
        checkObj.memberEmail = false; // 유효히지 않음으 일릴게..

        return;
    }

    // 5) 유효한 이메일 형식의 경우 중복 검사 수행
    // 비동기 요청(ajax)
    fetch("/member/checkEmail?memberEmail="+inputEmail)
    .then(resp =>resp.text())
    .then(count => {
        // count : 1이면 중독, 0이면 중복 X
        // == : 값이 같은지 (ex) "1" == 1 -> true
        ...
        if(count == 1) {    // 중복 O
            ...
            checkObj.memberEmail = false; // 유효히지 않은 상태
            return;
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    authKeyMessage.classList.remove("confirm", "error"); //글씨색 검정
    alert("인증번호가 발송되었습니다");

    // setInterval(콜백함수, 지연시간(ms))
    // - 지연시간(ms) 만큼 시간이 지날때 마다 콜백함수 수행
...
checkAuthKeyBtn.addEventListener("click", () => {
    ...
    if(authKey.value.length < 6 ||
        authKey.value.length >= 7){   // 인증번호가 제대로 입력 안된경우(길이가 6미만 또는 7이상)
    ...
    fetch("/email/checkAuthKey", {
        method: "POST",
        headers : {"Content-Type" : "application/json"},
        body : JSON.stringify(obj) // obj라는 JS객체를 JSON으로 변경한다.
    })
    .then ( resp => resp.text())
    .then ( result => {
        //1 or 0
...
memberPw.addEventListener("input", e => {
    ...
    // 비밀번호 입력 시 비밀번호 확인란의 값과 비료하는 코드 추가
...
signUpForm.addEventListener("submit", e => {
    ...
    for(let key in checkObj) {  // checkObj 요소의 key 값을 순서대로 꺼내옴
        ...
        if( !checkObj[key]) {   // 현재 접근중인 checkObj[key]의 value값이 false인 경우
