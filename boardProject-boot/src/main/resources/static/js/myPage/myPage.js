...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        updateInfo.submit();
        /* 
        submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수를 실행중일 때
...
//Img 태그에 작성하는 값 src = 미리보기 이미지를 띄울 URL주소
...
imageInput.addEventListener("change", () => {
    ...
    if(file) {  // 파일이 선택된 경우
        ...
        if(file.size <= MAX_SIZE){   // 현재 선택한 파일의 크기가 허용범위 이내인 경우(정상인경우)
        ...
        if(previousFile){   // 이전 파일이 존재한다면
            ...
            // DataTransfer를 이용하여 현재 File객체를 FileList 변환하여 할당 진행
            DataTransfer.items.add(previousFile);
...
deleteImage.addEventListener("click", () => {
    if(profileImg.src !== defaultImageUrl){
        imageInput.value = ""; // input 태그 파일값 처리
