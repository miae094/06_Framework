...
import edu.kh.project.email.model.contoller.EmailServiceImpl;
...
public class MyPageServiceImpl implements MyPageService {
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// - 현재 로그인한 회원의 암호화 된 비밀번호를 DB에서 조회
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 파일이 저장될 서버 폴더 경로
		String forderPath = "C:/uploadFiles/test/";
		
		// 클라이언트가 파일이 저장된 폴더에 접근할 수 있는 주소(정적리소스 요청 주소)
		String webPath = "/myPage/file/";
		
		// 2. DB에 전달할 데이터를 DTO 로 묶기
		...
		// forderPath경로(C:/uploadFiles/test/변경된파일명)으로
		// 파일을 서버 컴퓨터에 저장!
		uploadFile.transferTo(new File(forderPath+fileRename));
	...
	public int profile(MultipartFile profileImg, Member loginMember) throws Exception {
		...
		// 수정된 프로필 이미지 경로 + 회원번호를 저장할 DTO객체
