...
public class AdminController {
	...
	public ResponseEntity<Object> newMember() {
		try {
			List<Member> memberList = service.newMember();
	...
	public ResponseEntity<Object> selectWithdrawnMemberList(){
		// 성공시 List<Member> 반환, 실패 시 String 반환 -> Object 사용
	...
	public ResponseEntity<String> restoreBoard(@RequestBody Board board){
		try {
			...
			if(result > 0) {
				return ResponseEntity.status(HttpStatus.OK)
						.body(board.getMemberNo() + "번 회원 복구 완료");
			} else {
				// BAD_REQUEST : 400 -> 요청 구문이 잘못되었거나 유효하지 않음.
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
	...
	public ResponseEntity<String> createAdminAccount(@RequestBody Member member) {
		try {
			...
			// 2. 있으면 발급안함
			if(checkEmail > 0) {
				// HttpStatus.CONFLICT(409) : 요청이 서버의 현재 상태와 충돟할 때 사용
			...
			log.info("accountPw : " + accountPw);
			// HttpStatus.OK (200) : 요청이 정상적으로 처리되었으나기존 리소스에 대한 단순 처리
			// HttpStatus.CREATED(201) : 자원이 성공적으로 생성되었음을 나타냄
				return ResponseEntity.status(HttpStatus.CREATED) 
					.body(accountPw);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR) //500
					.body("관리자가 계정 생성 중 문제 발생(서버 문의 바람)");
		}
	}
	
	@GetMapping("selectManagerList")
	public ResponseEntity<Object> selectManagerList() {
		
		try {
			List<Member> memberList = service.selectManagerList();
