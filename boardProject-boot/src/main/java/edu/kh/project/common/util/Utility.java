...
public class Utility {
	...
	public static String fileRename(String originamFileName) {
	...
	public static String generatePassword() {
		...
		// 보안적으로 더 강력한 랜덤값을 생성함.
		// 일반적인 Random보다 예측 가능성이 낮아,
		// 민감한 데이터(암호생성) 와 같은 곳에 적합하다.
		...
		for(int i = 0; i < CODE_LENGTH; i++) {
			...
			// CHARACTERS.length(62) 는 0부터 61사이의 난수를 생성
			randomCode.append(CHARACTERS.charAt(index));
			// CHARACTERS 문자열의 index 위치에 있는 문자를 반환
			// ex) index가 0이면 'A', index가 61 이면 '9'를 반환
