...
public class FileConfig implements WebMvcConfigurer {
	...
	// 주로 웹 어플리케이션의 설정을 조정하거나 추가하는데 사용됨 (이미지 요청주소, 실제주소 매칭할 때 사용)
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		.addResourceLocations("file:///C:/uploadFiles/test/"); // 요청을 연결해서 처리해줄 서버 실제 폴더 경로
	...
	public MultipartConfigElement configElement() {
		...
		// 업로드 파일의 최대 크기, 메모리에서의 임시 저장 경로 등 설정 가능
		
		MultipartConfigFactory factory = new MultipartConfigFactory();
		
		// 파일 업로드 임계값
		factory.setFileSizeThreshold(DataSize.ofBytes(fileSizeThreshold));
		// DataSize.ofBytes() : long 타입을 전달하면 DataSize로 변환해줌
	...
	public MultipartResolver multipartResolver(){
		...
		// MultipartResolver 는 클라이언트로부터 받은 multipart 요청(문자, 숫자, 파일등이 섞인 요청)을 처리하고,
