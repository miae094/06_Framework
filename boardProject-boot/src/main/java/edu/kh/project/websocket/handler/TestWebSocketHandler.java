...
public class TestWebSocketHandler extends TextWebSocketHandler {
	...
	// WebSocketSession :  attributes.put("session", session); 얘임!
	// 클라이언트 - 서버 간 전이중(양방향) 통신을 담당하는 객체
	// SessionHandshakeInterceptor가 가로챈 연결한 클라이언트의 HttpSession 을 가지고 있음
	// attributes 에 put 으로 추가한 값을 가지고 있음
	...
	// 클라이언트로부터 텍스트 메세지를 받았을때 실행. 얘가 메인이 되는 메서드
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// 전달받은 메시지를 현재 해당 웹소켓에 연결되어있는 모든 클라이언트에게 보내기
