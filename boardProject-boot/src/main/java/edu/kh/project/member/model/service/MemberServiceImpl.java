...
public class MemberServiceImpl implements MemberService {
	...
	// 등록된 Bean 중에서 MemberMapper 와 같은 타입 or 상속관계인 Bean을 찾아
	// 의존성 주입(DI)
	@Autowired
	...
	public Member login(Member inputMember) {
		...
		// 3. 입력받은 비밀번호 (평문 : inputMember.getMemeverPw()) 와
		// 암호화된 비밀번호(loginMember.getMemberPw())
		// 두 비밀번호가 일치하는지 확인(bcypts.metches(평문, 암호화)
	...
	public int signup(Member inputMember, String[] memberAddress) {
		...
		if(!inputMember.getMemberAddress().equals(",,")) {
			...
			// 구분자로 "^^^" 쓴 이유 : 
			// -> 주소, 상세주소에 안쓰일것같은 특수 문자 작성
			// -> 나중에 마이페이지에서 주소 부분 수정시
			// -> DB에 저장된 기존 주소를 화면상에 출력해줘야함
			// -> 다시 3분할 해야할 때 구분자로 ^^^ 이용할 예정
			// -> 왜? 구분자가 기본 형태인 ,(콤마) 로 작성되어있으면
			// -> 주소, 상세주소에 ,(콤마) 가 들어오는 경우
