...
function getTotalCount() { // 함수 정의
    ...
    // fetch() API 코드 작성
    fetch("/ajax/totalCount")   // 서버로 "/ajax/totalCount" 로 GET 요청(기본값 GET)
    // 첫번째 then (응답을 처리하는 역할)
    .then(response => { // 서버에서 응답을 받으면, 
                        // 이 응답(response)을 텍스트 형식으로 반환하는 콜백함수
    ...
    // 두번째 then (첫번째 then 에서 return(변환)된 데이터를 활용하는 역할)    
    .then(result => { //첫번째 콜백 함수가 완료된 후 호출되는 콜백함수
                    // 변환된 텍스트 데이터(result)를 받아서
                    // 콘솔에 단순 출력
        // 매개변수 result : 첫번째 콜백함수에서 반환된 Promise 객체의 PrimiseReuslt 값
        // == result 매개변수로 받아서 처리
        console.log(result); // 최종 결과값


        // #totalCount 인 span 태그으 내용으로 result 값을 대입
...
function getCompleteCount(){
    ...
    .then(result => {
        // #conpleteCount 요소에 내용으로 result 값 출력
...
addBtn.addEventListener("click", () => {
    ...
    // todoTitle 과 todoContent 를 저장한 JS객체
    const param = {
        // key : value
    ...
    fetch("/ajax/add", {
        ...
        body : JSON.stringify(param) // param 이라는 JS객체를 JSON으로 변환
    })
...
const selectTodoList = () => {
    ...
    .then(resp => resp.json())  // 응답결과를 json 으로 받음
    .then(todoList => {
        // 매개변수 todoList:
        ...
        for(let todo of todoList) { // 향샹된 for 문
