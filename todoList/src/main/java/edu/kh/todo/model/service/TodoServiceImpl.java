...
// @Transactional
// - 트랜잭션 처리를 수행하라고 지시하는 어노테이션 (스프링한테 시킴. 개발자가 커밋/롤백 안해도됨)
// - 정상 코드 수행 시 COMMIT
...
@Service // 비즈니스 로직(데이터 가공, 트랜잭션 처리 등) 역할 명시 + Bean 등록 (MVC + IOC)
public class TodoServiceImpl implements TodoService {

	@Autowired	// TodoDAO와 같은 타입/상속관계 Bean 의존성 주입(DI)
	private TodoDAO dao;

	// DAO 가 하는 일이 없기 때문에 Service -> Mapper 로 바로 이동
	@Autowired
	...
	public int addTodo(String todoTitle, String todoContent) {
		...
		// 마이바티스에서 SQL에 전달 할 수 있는 파라미터 개수는 오직 1개!
		// -> todoTitle, todoContent 여러개인 파라미터를 전달하려면
		// 		Todo DTO로 묶어서 전달하면 된당
		Todo todo = new Todo();
