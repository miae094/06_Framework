 ...
 *  	DTO		<->    JSON		<-> JS Object
 * 
 * JSON : 서버와 클라이언트간에 데이터 전송할때 사용
 * 
 * (참고)
 * Spring 에서 HttpMessageConvertor 작동하기 위해서는
 * jackson-data-bind 라이브러리가 힐요한데
...
public class AjaxController {
	...
	@ResponseBody // 반환값을 HTTP 응답 본문으로 직접 전송( 값 그대로 돌려보낼거임!)
	@GetMapping("totalCount")
	public int getTotalCount() {
		
		// 전체 할 일 개수 조회 서비스 호출 결과 반환하기
