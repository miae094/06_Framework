...
import edu.kh.demo.DemoProjectApplication;
...
public class ParameterController {
    ...
    private final DemoProjectApplication demoProjectApplication;

    ParameterController(DemoProjectApplication demoProjectApplication) {
        this.demoProjectApplication = demoProjectApplication;
    }
	 ...
	 * -> required=true 인 파라미터가 존재하지 않는다면 400(Bed Request) 에러 발생
	 * -> "" (빈문자열) 일 때는 에러 발생 X(파라미터가 존재하지 않는것이 아니라 name 속성값="" 로 넘어오기 때문에.)
	 * 
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							@RequestParam("price") int price, // 형변환이 필요한 경우에는 파라미터 value값 필수 작성 되어있어야 함.
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		// -> 같은 name 속성을 가진 파라미터는 String[], List 로 저장되는게 아니라
		//	  첫번째로 제출된 value 값이 저장됨
		log.debug("paramMap : " + paramMap);
		// paramMap : {color=Red, fruit=Apple, productName=키키오렌지, expirationDate=2025-04-25}
	...
	// 전달받은 파라미터의 name 속성값이 함께 사용되는 DTO의 필드명과 같다면
	// 자동으로 setter 를 호출해서 필드에 값을 저장
	
	// ** 주의사항 **
	// - DTO 에 기본생성자가 필수로 존재해야 한다.
	// - DTO 에 setter 가 필수로 존재해야 한다.
	
	// @ModelAttribute 를 이용해 값이 필드에 세팅된 객체를
	// "커맨드 객체" 라고 부른다
	
	// @ModelAttribute 생략가능!
	// @ModelAttribute MemberDTO inputMember => MemberDTO inputMember 이렇게 써도 된다는 뜻
	@PostMapping("test4")
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		// == "redirect:/param/main"
		//(현재 위치가 중요!
		// -> 현재 경로의 가장 마지막 부분을 redirect:이주소 로 변경하기 때문!)
		
		// 만약 "redirect"test/result" 라면 -> /param/test/result 가되는것!
		}
